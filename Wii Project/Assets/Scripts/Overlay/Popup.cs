...
{
    //script for the overlay in the scenes level 1, 2 and 3
    ...
    public BalanceBar failRestart; //check in BalanceBar script if the level was failed and needs to reset

    public string eventTrigger; //communicate what needs to happen

    public Boat boat; //call in boat from Boat script
    ...
    //if the level is restarted, the countdown is reset, and the boat is told it needs to start again
    void Restart() {
        popup.SetActive(false);
        failure.SetActive(false);
        failRestart.failureRestart = false;
